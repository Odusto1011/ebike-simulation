...
class BikePhysicsModel:
    ...
    def calculate(self, route: pd.DataFrame) -> pd.DataFrame:
        ...
        Die Luftdichte wird für jeden Streckenpunkt aus der Höhe über
        dem Meeresspiegel und der Umgebungstemperatur bestimmt.
        """
        ...
        # RouteAnalyzer erzeugt diese geglättete Höhenspalte.
        altitude = df["elevation_filtered_m"].to_numpy(dtype=float)

        air_density = self.calculate_air_density(
            altitude_m=altitude,
            temperature_c=cfg.ambient_temperature_c,
        )
        ...
        # Negative Kraft bedeutet Bremsen beziehungsweise Unterstützung
        # durch Gefälle.
    ...
    def calculate_air_density(
        ...
        temperature_c:
            Lufttemperatur in Grad Celsius. Ohne Angabe wird die
            Umgebungstemperatur aus SimulationConfig verwendet.
        ...
        if np.any(pressure_base <= 0.0):
            raise ValueError(
                "Mindestens eine Höhe liegt außerhalb des gültigen "
